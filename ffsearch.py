...
# TODO Add  FFSearchWithEngine so users can type text then pick engine
# TODO Handle JSON based search plugins like virustotal
...
def search_engines_from_json(sjson):
    ...
    return [(e["_name"], e["description"], e["_urls"][0]["template"]) for e in engines]
...
class FFSearchSource(Source, FilesystemWatchMixin):
    ...
    def get_items(self):
        search_engines = search_engines_from_json(get_search_json(self.ff_dir))
        return [SearchEngine(name, desc, url) for (name, desc, url) in search_engines]
